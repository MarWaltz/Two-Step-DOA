...
class ObstacleAvoidance(gym.Env):
    def __init__(self, 
        ...
        self.observation_space = spaces.Box(low=np.full((1, self.frame_stack * (num_vessel_obs * self.n_vessels + 3)), 
                                                        -1, dtype=np.float32)[0],
                                            high=np.full((1, self.frame_stack * (num_vessel_obs * self.n_vessels + 3)),
                                                          1, dtype=np.float32)[0])
    ...
    def _set_dynamics(self):
        ...
        self.agent_vx = 5
        self.x_max = np.ceil(self.agent_vx * self.max_episode_steps * self.delta_t)
        ...
        if self.setting == "EST":
            ...
            if self.current_timestep == 1 or self.current_timestep % 10 == 0: 
                for i in range(self.FORECAST_WIN):
                    ...
                    # extract last trajectory points
                ...
                # smoothing dist curve by using moving average
                kernel_size = 21
                smoothed_dist = np.ones_like(dist) * self.dcpa_scale * 10  # init with high values 
                for i, d in enumerate(dist):
                    smoothed_dist[i][int((kernel_size-1)/2):-int((kernel_size-1)/2)] = np.convolve(d, np.ones(kernel_size)/kernel_size, 'valid')
